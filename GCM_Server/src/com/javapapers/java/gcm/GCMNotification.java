...
public class GCMNotification extends HttpServlet {
	...
	private static final String GOOGLE_SERVER_KEY = "AIzaSyC3hK5uFiFIaz7VQ8e4RC_a59y32_SGUwc";
	...
	protected void doPost(HttpServletRequest request,
		...
		if ("shareRegId".equalsIgnoreCase(action)) {
			...
			try {
				String fromName = request.getParameter(REGISTER_NAME);
				String userMessage = request.getParameter(MESSAGE_ACTIVITY);
				Sender sender = new Sender(GOOGLE_SERVER_KEY);
				Message message = new Message.Builder().timeToLive(30)
						.delayWhileIdle(true).addData(MESSAGE_ACTIVITY, userMessage)
						.addData(REGISTER_NAME, fromName).build();
				Map<String, String> regIdMap = readFromFile();
	...
	private Map<String, String> readFromFile() throws IOException {
                File f = new File("GCMRegId.txt");
                if(!f.exists()){
                PrintWriter writer = new PrintWriter("GCMRegId.txt", "UTF-8");
                }
		...
		while ((regIdLine = br.readLine()) != null ) {
			String[] regArr = regIdLine.split(",");
                        if(!regArr[0].equalsIgnoreCase("null")){
